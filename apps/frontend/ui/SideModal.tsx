...
import { configureStepDetails, onStep } from "@/RecoilState/currentZap";
...
export default function SideModal({
  ...
  useEffect(() => {
    async function getUserInfo() {
      ...
      setUser(response?.user as unknown as SessionType);
    }
    getUserInfo();
  }, [setUser]);
  ...
  const handleContinue = async () => {
    if (!CheckStepValidity(panelIndex, index)) return;
    if (metaData.index == null) return;

    let userId = user?.userId;
    if (!user) {
      const session = await getSession();
      setUser(session?.user as unknown as SessionType);
      userId = (session?.user as unknown as SessionType)?.userId;
    }
          ...
          !!zap.selectedItems[index].metadata.optionConfiguration[configureId]
            ...
            <StepsStatus
              ...
              step={
                zap.selectedItems[index].metadata.optionConfiguration[
                  configureId
                ].testStep as unknown as itemStepMetaData
              }
              setIndex={setpanelIndex}
            />
          ) : (
