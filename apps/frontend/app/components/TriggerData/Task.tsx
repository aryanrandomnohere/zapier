...
export default function Task({item, imagePath}:{item:itemTestMetaData, imagePath:string}) {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(fetchedUrl || "https://hooks.zapier.com/hooks/c...");
    setIsPressed(true);
...
<div className={`flex justify-between ${isOpen ? "flex-col" : ""} `}>
      ...
      <div className="text-xs text-gray-600 ">
        <p className="">
          { isOpen ? item.userTriggered?.description : item.userTriggered?.description.slice(0,36)+ "..." }
        </p>
        {isOpen && <a 
