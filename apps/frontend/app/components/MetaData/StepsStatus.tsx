...
export default function StepsStatus({ step, key}: { step: itemStepMetaData, key: number }) {
  ...
  return (
    <div key={key} className={`flex items-center text-sm gap-1 px-3 py-1.5 rounded-md ${isActive ? ' text-blue-700' : isCompleted ? ' text-gray-600' : ' text-gray-600'}`}>
      <span className="text-sm font-medium">{step.stepName}</span>
      {step.completed === null ? 
        <div className="text-blue-500"><IoTimerOutline size={18} /></div> : 
        step.completed ? 
          <div className="text-green-500"><GoCheckCircleFill size={18} /></div> : 
          <div className="text-yellow-500 bg-black rounded-full"><BiSolidErrorCircle size={18} /></div>
      }
    </div>
  )
}
