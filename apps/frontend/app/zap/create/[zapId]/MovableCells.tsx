...
import { configureStepDetails, onStep, selectedItemMetaData } from "@/app/RecoilState/currentZap";
import { onStepEnum } from "@repo/types";
import { ZapCreateState } from "../../../RecoilState/store/zapCreate";
    export default function MovableCells( {zapState, setZapState }: {zapState: ZapCreateState, setZapState:SetterOrUpdater<ZapCreateState>} ) {
  ...
  >
    <div className="absolute flex flex-col top-1/2 left-4/9  -translate-x-2/6 -translate-y-2/4">
      ...
      !zapState.selectedItems[0]?.name ? (
        ...
        <div onClick={() => { if (!metaData.isOpen) { handleSetMetaData(0); } }}>
          {" "}
          <ZapCell
            copiedItem={copiedItem}
            setCopiedItem={setCopiedItem}
