"use client";
import { itemStepMetaData, ItemType, onStepEnum } from "@repo/types";
...
export default function OptionList({
  ...
  function handleClick(item: ItemType | MockItem) {
    ...
    // Open metadata modal
    setMetaData(() => ({ index: finalIndex, isOpen: true }));
  ...
  return (
    <div className="flex flex-col gap-1 w-full">
      ...
      {items?.map((item: ItemType | MockItem) => (
        <div
          onClick={() => {
            if (
